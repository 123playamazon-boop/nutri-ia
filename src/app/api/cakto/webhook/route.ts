...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existing) {
      ...
      // ENTREGA DE ACESSO para contas que ja existiam mas NUNCA logaram (conta
      // antiga sem senha conhecida, ou cliente que nunca acessou). Sem isto o
      // cliente paga, a assinatura reativa, mas ele nao recebe o login.
      // Dispara no maximo 1x (welcome_sent) e NUNCA para quem ja logou - assim
      // nao reseta senha de cliente ativo nem duplica e-mail nos upsells.
      const meta = (existing.user_metadata ?? {}) as Record<string, unknown>;
      const neverLoggedIn = !existing.last_sign_in_at;
      const welcomeSent = meta.welcome_sent === true;
      let emailSent = false;
      if (neverLoggedIn && !welcomeSent) {
        const tempPassword = generateTemporaryPassword();
        await admin.auth.admin.updateUserById(existing.id, {
          password: tempPassword,
          user_metadata: { ...meta, must_change_password: true, welcome_sent: true, source: "cakto" },
        });
        try {
          await sendWelcomeCredentialsEmail({ to: email, password: tempPassword, loginUrl });
          emailSent = true;
        } catch (mailErr) {
          console.error("Cakto webhook: e-mail (reativacao) falhou, conta reativada mesmo assim:", mailErr);
        }
      }
      return NextResponse.json({ received: true, action: "reactivated", emailSent });
    }
    ...
    const { data: created, error } = await admin.auth.admin.createUser({
      ...
      password: tempPassword,
      email_confirm: true,
      user_metadata: { must_change_password: true, welcome_sent: true, source: "cakto", name },
    });
