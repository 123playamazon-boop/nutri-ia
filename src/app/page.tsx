...
export default function Page() {
  ...
  const realAge = parseInt(age || "0", 10);
...
function DataStep({ dataStep, name, age, height, weight, setName, setAge, setHeight, setWeight, onNext }: {
  ...
  const ok = dataStep === 0 ? (name.trim().length > 0 && parseInt(age || "0", 10) > 0) : (parseInt(height || "0", 10) > 0 && parseInt(weight || "0", 10) > 0);
